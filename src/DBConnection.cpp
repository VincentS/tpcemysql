...
//Set Schema for HANA Database for each connection (Default:TPCE)
    ...
    select_schema << "SET SCHEMA "<< szSName;
    rc = SQLExecDirect(m_Stmt, (SQLCHAR*)(select_schema.str().c_str()), SQL_NTS);
    if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
        ThrowError(CODBCERR::eExecDirect, SQL_HANDLE_STMT, m_Stmt, __FILE__, __LINE__);

    rc = SQLExecDirect(m_Stmt2, (SQLCHAR*)(select_schema.str().c_str()), SQL_NTS);
    if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
